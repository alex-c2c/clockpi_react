...
export default function ModalScheduleUpdate({
	...
	const [scheduleId] = useState<number>(selectedSchedule.id);
	...
	const handleClickUpdate = async () => {
		...
		const result: Result<void> = await fetchScheduleUpdate(deviceId, currentSchedule);
		if (!result.success) {
	...
	const handleClickDelete = async () => {
		const result: Result<void> = await fetchScheduleDelete(deviceId, scheduleId);
		if (!result.success) {
			setError(result.error);
		} else {
			setIsOpen(false);
